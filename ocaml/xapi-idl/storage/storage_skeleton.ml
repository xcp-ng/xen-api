...
module DATA = struct
  ...
  module MIRROR = struct
    ...
    let receive_start ctx ~dbg ~sr ~vdi_info ~image_format ~id ~similar =
      Storage_interface.unimplemented __FUNCTION__

    let receive_start2 ctx ~dbg ~sr ~vdi_info ~image_format ~id ~similar ~vm =
      Storage_interface.unimplemented __FUNCTION__

    let receive_start3 ctx ~dbg ~sr ~vdi_info ~image_format ~mirror_id ~similar
        ~vm ~url ~verify_dest =
      Storage_interface.unimplemented __FUNCTION__
