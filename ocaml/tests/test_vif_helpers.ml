...
let create ~__context ~device ~network ~vM ?(mAC = "00:00:00:00:00:00")
    ...
    ?(ipv4_gateway = "") ?(ipv4_dns = []) ?(ipv6_configuration_mode = `None)
    ?(ipv6_addresses = []) ?(ipv6_gateway = "") ?(ipv6_dns = []) ?(trunks = [])
    () =
  Xapi_vif_helpers.create ~__context ~device ~network ~vM ~mAC ~mTU
    ...
    ~ipv4_addresses ~ipv4_dns ~ipv4_gateway ~ipv6_configuration_mode
    ~ipv6_addresses ~ipv6_gateway ~ipv6_dns ~trunks
