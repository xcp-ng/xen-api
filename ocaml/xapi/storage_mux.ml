...
module Mux = struct
  ...
  module DATA = struct
    ...
    module MIRROR = struct
      ...
      let receive_start () ~dbg ~sr ~vdi_info ~image_format ~id ~similar =
        with_dbg ~name:"DATA.MIRROR.receive_start" ~dbg @@ fun _di ->
        info
          "%s dbg: %s sr: %s vdi_info: %s image_format: %s mirror_id: %s \
           similar: %s"
        ...
        Storage_smapiv1_migrate.MIRROR.receive_start () ~dbg ~sr ~vdi_info
          ~image_format ~id ~similar

      let receive_start2 () ~dbg ~sr ~vdi_info ~image_format ~id ~similar ~vm =
        ...
        (* This goes straight to storage_smapiv1_migrate for backwards compatability
           reasons, new code should not call receive_start any more *)
        Storage_smapiv1_migrate.MIRROR.receive_start2 () ~dbg ~sr ~vdi_info
          ~image_format ~id ~similar ~vm

      (** see storage_smapiv{1,3}_migrate.receive_start3 *)
      let receive_start3 () ~dbg:_ ~sr:_ ~vdi_info:_ ~image_format:_
          ~mirror_id:_ ~similar:_ ~vm:_ =
