...
module MIRROR : SMAPIv2_MIRROR = struct
  ...
  let send_start _ctx ~dbg ~task_id:_ ~dp ~sr ~vdi ~image_format ~mirror_vm
      ...
      ~verify_dest =
    D.debug
      "%s dbg: %s dp: %s sr: %s vdi:%s image_format: %s mirror_vm:%s \
       mirror_id: %s live_vm: %s url:%s dest_sr:%s verify_dest:%B"
    ...
    | Mirror.SMAPIv3_mirror {nbd_export; mirror_datapath; mirror_vdi} -> (
      try
        ...
        D.info "%s nbd_proxy_path: %s nbd_url %s" __FUNCTION__ nbd_proxy_path
          nbd_uri ;
        let mk =
          Local.DATA.mirror dbg sr vdi "GTNDEBUG: image_format" live_vm nbd_uri
        in
  ...
  let receive_start _ctx ~dbg:_ ~sr:_ ~vdi_info:_ ~image_format:_ ~id:_
      ~similar:_ =
    Storage_interface.unimplemented __FUNCTION__

  let receive_start2 _ctx ~dbg:_ ~sr:_ ~vdi_info:_ ~image_format:_ ~id:_
      ~similar:_ ~vm:_ =
    Storage_interface.unimplemented __FUNCTION__

  let receive_start3 _ctx ~dbg ~sr ~vdi_info ~image_format:_ ~mirror_id
      ~similar:_ ~vm ~url ~verify_dest =
