...
let choose_pbds_for_sr ~consider_unplugged_pbds ~__context ~self () =
  ...
  let pbds_to_consider =
    if consider_unplugged_pbds then
      all_pbds
    else
      plugged_pbds
  in
    ...
    module VIF = struct
      ...
      let configure_ipv6 ~__context ~self ~mode ~address ~gateway ~dns =
        info
          ...
          address gateway (String.concat "; " dns) ;
        let local_fn =
          Local.VIF.configure_ipv6 ~self ~mode ~address ~gateway ~dns
        in
        let remote_fn =
          Client.VIF.configure_ipv6 ~self ~mode ~address ~gateway ~dns
        in
        forward_vif_op ~local_fn ~__context ~self ~remote_fn
    ...
    module PBD = struct
      ...
      let unplug ~__context ~self =
        ...
        with_unplug_locks ~__context ~sr ~pbd:self (fun () ->
            if is_master_pbd then Xapi_sr.maybe_copy_sr_rrds ~__context ~sr ;
            forward_pbd_op ~local_fn ~__context ~self ~remote_fn
        )
    end
